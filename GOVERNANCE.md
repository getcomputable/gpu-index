...
A provider named in the observatory can write to team@getcomputable.com.
Collection for a named source pauses within five business days of a verified
request from that provider. Roster rows are marked retired, never deleted, and
published history stays as the record; on request, per-receipt price
disclosure for that source can be withheld in future artifacts.
