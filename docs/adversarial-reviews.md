...
| adversarial review 2026-08-27 | The pre-landing review of the minute-lattice re-base. Confirmed findings, each with a pinned test: duplicate-slot selection orders by the RUN ID that follows the slot token, never raw key bytes (`slot16-` sorts before `slot1600-`, so a later hour-token capture would otherwise have beaten an earlier minute-token one for the same mark); the slot-idempotency probe checks BOTH token formats in BOTH writer directions, so an hour-vocabulary writer running after minute keys exist finds the capture instead of double-capturing; a minute-keyed panel lane refuses to RUN without `PANEL_MINUTE_LANES_LIVE=true` while `--check-config` stays fence-free; the period-rate CLI refuses minute-form period boundaries on an hour-keyed lane at the parse site (typed refusal) rather than crashing later in report assembly; and flip-day alarm honesty — previous-day marks before the day's first capture get a grid-change notice instead of days of false 'permanently lost' warnings burying real interior misses. | `src/gpu_index/common/store.py`, `src/gpu_index/common/slots.py`, `scripts/compute_panel_index.py`, `scripts/compute_period_rate.py`, `scripts/capture_raw_observatory.py`, `tests/unit/test_minute_lattice.py` |
